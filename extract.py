...
def straighten(image):
	...
	ctrs,hier = cv2.findContours(dilated.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2:]
	...
	for i, ctr in enumerate(ctrs):
		...
		countour_count += 1
		if (angle >= 1):
			check_angle_median.append(0)
		elif (angle <= -0.5):
			check_angle_median.append(1)
		else:
			check_angle_median.append(2)
	...
	print("Contour count=",countour_count)
	print(check_angle_median)
	#print("Mode angle is: ",stats.mode(check_angle_median))
	mean_angle = angle_sum / countour_count
	BASELINE_ANGLE = mean_angle
	print("Mean angle is: ", BASELINE_ANGLE)
...
def start(file_name):
	...
	thresh, image = cv2.threshold(img2, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)


	cv2.imwrite("Images/Rescaled.jpg",image)
	image = cv2.imread(file_name)
