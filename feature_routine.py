...
if os.path.isfile("feature_list"):
	...
	with open("raw_feature_list", "r") as raw_features, open("feature_list", "a") as features:
		for line in raw_features:
			...
			baseline_angle = categorize.determine_baseline_angle(raw_baseline_angle)
			top_margin = categorize.determine_top_margin(raw_top_margin)
			letter_size = categorize.determine_letter_size(raw_letter_size)
			line_spacing = categorize.determine_line_spacing(raw_line_spacing)
			word_spacing = categorize.determine_word_spacing(raw_word_spacing)
			pen_pressure = categorize.determine_pen_pressure(raw_pen_pressure)
			slant_angle = categorize.determine_slant_angle(raw_slant_angle)
