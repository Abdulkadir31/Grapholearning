...
def run_app(file_path):
    ...
    if os.path.isfile("label_list"):
        ...
        baseline_angle, comment, description, keyword = categorize.determine_baseline_angle(raw_baseline_angle)
        ...
        top_margin, comment, description, keyword = categorize.determine_top_margin(raw_top_margin)
        ...
        letter_size, comment, description, keyword = categorize.determine_letter_size(raw_letter_size)
        ...
        line_spacing, comment, description, keyword = categorize.determine_line_spacing(raw_line_spacing)
        ...
        word_spacing, comment, description, keyword = categorize.determine_word_spacing(raw_word_spacing)
        ...
        pen_pressure, comment, description, keyword = categorize.determine_pen_pressure(raw_pen_pressure)
        ...
        slant_angle, comment, description, keyword = categorize.determine_slant_angle(raw_slant_angle)

        keywords.append(keyword)
        descriptions.append(description)


        career.append(top_margin)
        career.append(letter_size)

        career.append(line_spacing)
        career.append(pen_pressure)
        career.append(slant_angle)
        ...
        print("Keywords of your personality: ", keywords)

        print("In Depth:")
        descrip =""
        for i, d in enumerate(descriptions):
            descrip = descrip+d+"\n\n"
        ans = career_predict.career(career)
        careers_result = ""
        for i in ans:
            careers_result = careers_result+"#"+i

        return descrip,careers_result
